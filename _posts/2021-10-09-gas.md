...
excerpt: Demistifying the Gameplay Ability System
author: Meta
...
As we can see, the tag system shifts the paradigm towards stateful systems, by providing a simple controllability interface in our abilities. In traditional systems this would be equivalent to a long and complicated **if-else** boolean statement chain.
...
Numbered nodes explanation:
  ...
  2. ActorInfo holds all the information relevant to our Ability System Component. Within this information we can find the avatar, which represents the Actor that actuates over the AbilityComponent.
  ...
  5. Similar to the WaitGameplayTagAdd node, it waits until the specified gameplay tag event is triggered. Right after the execution of this node the blueprint calls _Enter Charging State_. If the event is received we'll perform a smooth rotation over Echo's root.

### 3. Charging State

Following this, the character will begin to levitate, spawning particles and triggering sounds, for that, this project uses the gameplay cue system. When this happens, Echo reduces their movement speed and starts casting the _Light Dart_.
...
Like everything else, when it comes to code design, it is just a matter of learning the tools provided by the system and using them properly to solve the different problems you will face:
