...
As we can see above, `OnGiveAbility` is called when the local client receives the ability, so wen can start manipulating it locally and therefore binding them to an InputAction.
